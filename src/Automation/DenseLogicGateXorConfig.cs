...
namespace Automation
{
    public class DenseLogicGateXorConfig : DenseLogicGateBaseConfig
    {
        ...
        {
            var buildingDef = CreateBuildingDef(Id, "xor_dense_kanim", 2, 2);
            buildingDef.LogicInputPorts = new List<LogicPorts.Port>()
            {
                LogicPorts.Port.RibbonInputPort(PixelPack.PORT_ID, new CellOffset(0, 0),
                ...
                LogicPorts.Port.RibbonInputPort(PixelPack.PORT_ID, new CellOffset(0, 1),
                    UI.LOGIC_PORTS.GATE_MULTI_INPUT_ONE_NAME,
                    UI.FormatAsAutomationState("Green Signal", UI.AutomationState.Active) + ": Provides a " + UI.FormatAsAutomationState("Green Signal", UI.AutomationState.Active) + " to its corresponding bit in the AND gate.",
                    UI.FormatAsAutomationState("Red Signal", UI.AutomationState.Standby) + ": Provides a " + UI.FormatAsAutomationState("Red Signal", UI.AutomationState.Standby) + " to its corresponding bit in the AND gate."),
            };
            ...
            {
                LogicPorts.Port.OutputPort(PixelPack.PORT_ID, new CellOffset(1, 0),
                    BUILDINGS.PREFABS.LOGICGATEXOR.OUTPUT_NAME,
                    BUILDINGS.PREFABS.LOGICGATEXOR.OUTPUT_ACTIVE,
                    BUILDINGS.PREFABS.LOGICGATEXOR.OUTPUT_INACTIVE, true, false)
            };
            return buildingDef;
        }
    }
